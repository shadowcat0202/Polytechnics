...
img_resize_rate = 0.6 # 원본 이미지에서 50%로 줄여도 detection에서는 크게 문제가 없다 640x400
if video_capture.isOpened():
    ...
    while True:
        ...
        cmpos = ps.img_Preprocessing_v2(img)  # 프레임별 이미지 전처리
        ...
        if face_box is not None:
            ...
            if face_box1 is not None:
                detection_frame1 += 1
                landmark1 = mark_detector.get_marks(dst, face_box1)
                # 눈 랜드마크 그려주기
                mark_detector.draw_marks(dst, landmark1[36:48], color=BLACK)
                cv2.putText(dst,f"{round((detection_frame1 / total_frame) *100, 2)}", (30,30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, WHITE, 2)
                cv2.imshow("dst", dst)
            # # 눈 감는거 + 자는거(부 정확함) 판단=======================================================
            ...
            # # 얼굴 랜드마크 보고 싶다면?
            # mark_detector.draw_marks(img, landmark[36:48], color=WHITE)
