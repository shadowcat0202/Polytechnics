...
class PoseEstimator:
    ...
    def __init__(self, img_size=(480, 640), model_path='assets/model.txt'):
        ...
        #     (150.0, -150.0, -125.0)  # Mouth right corner
        # ])
        print(f"loading pose_estimator model {model_path}")
        self.model_points_68 = self._get_full_model_points(filename=model_path)
        print(f"complete load pose_estimator model!")

        # Camera internals
        print(f"camera internals setting...")
        ...
        self.camera_matrix = np.array(
            [[self.focal_length, 0, self.camera_center[0]],
             ...
             [0, 0, 1]], dtype="double")
        print(f"complete camera internals setting!")
