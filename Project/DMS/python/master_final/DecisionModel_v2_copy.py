...
class DecisionModel:
    ...
    def Update(self, img, eyeClose, headDrop, gaze, headDir):
        ...
        warning
        """
        cv2.putText(img, f"eye_close:{eyeClose}", (1000, 200), cv2.FONT_HERSHEY_PLAIN, fontScale=1, color=(255, 0, 0), thickness=2)
        # cv2.putText(img, f"head_drop:{head_drop}", (1000, 220), cv2.FONT_HERSHEY_PLAIN, fontScale=1, color=(255, 0, 0), thickness=2)
        cv2.putText(img, f"non_gaze:{non_gaze}", (1000, 220), cv2.FONT_HERSHEY_PLAIN, fontScale=1, color=(255, 0, 0), thickness=2)
        # cv2.putText(img, f"head_side:{head_side}", (1000, 220), cv2.FONT_HERSHEY_PLAIN, fontScale=1, color=(0, 0, 255), thickness=1)
