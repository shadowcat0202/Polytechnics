...
if video_capture.isOpened():
    ...
    while True:
        ...
        if detection:
            ...
            # x1 = detection[d_index].left()
            # y1 = detection[d_index].top()
            # x2 = detection[d_index].right()
            # y2 = detection[d_index].bottom()
            # face_img = np.array([(x1, y1), (x2, y2)])

            shape = shape_predictor(cmpos, detection[d_index])  # 그 얼굴에서 랜드마크 추출
            # pose_estimator.draw_annotation_box(
            #     img, pose[0], pose[1], color=(0, 255, 0))
