...
for dataset in combine:
    dataset['Cabin_c'] = dataset['Cabin_c'].map(cabin_c_mapping)
...
with tf.device("/device:GPU:0"):
    f_logistic_model.fit(x=X_train, y=Y_train, epochs=50, batch_size=10)
...
cnn_prediction = pd.DataFrame(cnn_prediction, columns=["Survived"])
cnn_prediction.loc[cnn_prediction['Survived'] >= 0.5, 'Survived'] = int(1)
cnn_prediction.loc[cnn_prediction['Survived'] < 0.5, 'Survived'] = int(0)
cnn_prediction = cnn_prediction.values.tolist()
submission = pd.DataFrame({
    "PassengerId": pid,
    "Survived": cnn_prediction
})
# submission["Survived"] = submission["Survived"].astype(int)
# print(submission.info())
print("CNN", score(submission))