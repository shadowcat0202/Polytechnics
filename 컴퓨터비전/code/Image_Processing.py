...
def change_raw_col(_img):
    _img[round(_img.shape[0] / 2), :] = green
    _img[:, round(_img.shape[1] / 2)] = blue
...
def my_Threshold(_img, threshold=100):
    """
    2진 threshold
    :param _img: gray_img
    ...
    for row in range(bin_img.shape[0]):
        for col in range(bin_img.shape[0]):
